...
namespace dx11
{
    ...
    Texture* ResourceManager::LoadTexture(const WCHAR* filename)
    {
        ...
        auto hr = DirectX::LoadFromWICFile(filename, DirectX::WIC_FLAGS_NONE, &metadata, scratchImage);
        if (SUCCEEDED(hr))
        {
        ...
        {
            hr = DirectX::CreateDDSTextureFromFile(Context::Instance().device.Get(), filename, texture->texture.GetAddressOf(), texture->srv.GetAddressOf());
            texturesByPath[filename] = std::move(texture);
        }
        assert(SUCCEEDED(hr) && "error while loading texture file!");
