...
namespace dx11
{
    ...
    bool Context::OnResize()
    {
        ...
        // 기본 뎁스 스텐실 상태
        {
            ...
            hr = device->CreateDepthStencilState(&desc, defaultDepthStencilState.GetAddressOf());
            assert(SUCCEEDED(hr));
        }

        // less equal 뎁스 스텐실 상태
        {
            D3D11_DEPTH_STENCIL_DESC desc{
                .DepthEnable = TRUE,
                .DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL,
            ...
            hr = device->CreateDepthStencilState(&desc, lessEqualDepthStencilState.GetAddressOf());
            assert(SUCCEEDED(hr));
        }
        ...
        // 기본적인 백페이스 컬링을 하는 레스터라이저
        {
            ...
            device->CreateRasterizerState(&rasterizerDesc, defaultRasterizerState.GetAddressOf());
        }
        // 기본적인 백페이스 컬링조차 하지 않는 레스터라이저
        {
            D3D11_RASTERIZER_DESC rasterizerDesc
            {
                .FillMode = D3D11_FILL_SOLID,
            ...
            device->CreateRasterizerState(&rasterizerDesc, nonCullingRasterizerState.GetAddressOf());
        }
    ...
    bool Context::UpdateCameraCbuffer()
    {
        ...
        {
            .view{Camera::GetMainCam()->GetVTM()},
            .world{Camera::GetMainCam()->GetWTM()},
            .projection{Camera::GetMainCam()->GetPTM()},
            .viewProjection{viewMatrix* Camera::GetMainCam()->GetPTM()},
        };
    ...
    void Context::TestInitCode()
    {
        ...
        cubemap->SetCubeMapMesh(ResourceManager::Instance().LoadFBXFile("Sphere.fbx")->mMeshes[0]);
        cubemap->SetTexture(ResourceManager::Instance().LoadTexture(L"room.dds"));
    }
    ...
    void Context::TestUpdateCodeTriangle()
    {
        ...
        static auto vertexShader = ResourceManager::Instance().LoadVertexShaderFromFile(L"yd11ColoredVertexShader.cso");
